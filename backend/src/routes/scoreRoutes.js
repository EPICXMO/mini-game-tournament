...
// =============================================================================
// CLASH ROYALE MINI-GAME ENDPOINTS
// =============================================================================

/**
 * POST /api/score/clashroyale - Submit a Clash Royale game score
 */
router.post('/clashroyale', async (req, res) => {
  try {
    const { score, distance, coins, time, userId, metadata } = req.body;
    ...
    const scoreData = {
      ...
      distance: distance ? Math.floor(distance) : 0, // Tower damage dealt
      coinsCollected: coins ? Math.floor(coins) : 0, // Remaining elixir
      gameTime: time || 0,
      metadata: {
        ...metadata, // Include game-specific metadata (efficiency, damage taken, etc.)
        userAgent: req.headers['user-agent'],
        timestamp: new Date().toISOString()
      }
    };
    ...
    res.status(201).json({
      success: true,
      message: 'Clash Royale score submitted successfully',
      data: {
        scoreId: result.id,
        score: result.score,
        towerDamage: result.distance,
        elixirRemaining: result.coinsCollected,
        time: result.gameTime,
        submittedAt: result.createdAt
      }
    });
    ...
    console.error('Error submitting Clash Royale score:', error);
...
/**
 * GET /api/score/clashroyale/leaderboard - Get Clash Royale leaderboard
 */
router.get('/clashroyale/leaderboard', async (req, res) => {
  try {
    const limit = Math.min(parseInt(req.query.limit) || 10, 50); // Max 50 entries

    const leaderboard = await ScoreService.getLeaderboard('clashroyale', limit);
    ...
    console.error('Error getting Clash Royale leaderboard:', error);
...
/**
 * GET /api/score/clashroyale/stats - Get Clash Royale game statistics
 */
router.get('/clashroyale/stats', async (req, res) => {
  try {
    const stats = await ScoreService.getGameStats('clashroyale');
    ...
    console.error('Error getting Clash Royale stats:', error);
...
/**
 * GET /api/score/clashroyale/user/:userId - Get user's Clash Royale scores
 */
router.get('/clashroyale/user/:userId', async (req, res) => {
  try {
    const userId = parseInt(req.params.userId);
    const limit = Math.min(parseInt(req.query.limit) || 5, 20); // Max 20 entries
    ...
    // Get both best score and recent scores
    const [bestScore, recentScores] = await Promise.all([
      ScoreService.getUserBestScore(userId, 'clashroyale'),
      ScoreService.getUserRecentScores(userId, 'clashroyale', limit)
    ]);
    ...
    console.error('Error getting user Clash Royale scores:', error);
